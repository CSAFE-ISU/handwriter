...
processHandwriting = function(img, dims)
{
 ...
 # print(plotPath(candidateNodes, img, img, zoomBorder = NA))
  ...
  finalBreaks = finalBreaks[!(checkSimplicityBreaks(finalBreaks, pathList, letters, skel_graph0, nodeList, terminalNodes, hasTrough, dims))]
...
checkSimplicityBreaks = function(candidateBreaks, pathList, letters, nodeGraph0, nodeList, terminalNodes, hasTrough, dims)
{
  ...
  for(i in 1:length(allPaths))
  {
    ...
    if(length(nodestoCheck) >= 1)
    {
      if(!hasTrough[i])
      {
        ...
        {
          pathsOnLeft = length(pathLetterAssociate(pathList,left))
          pathsOnRight = length(pathLetterAssociate(pathList, right))
